...
public class TelemetryController {
    ...
    @PostMapping("/devices/{deviceId}/telemetry-service")
    public ResponseEntity<Void> saveTelemetry(
        ...
        log.info(String.format("create telemetry-service for device id '%s'", deviceId));
        return ResponseEntity.status(HttpStatus.CREATED).build();
    }

    @GetMapping("/devices/{deviceId}/telemetry-service/latest")
    public ResponseEntity<TelemetryDto> getLatestTelemetry(@PathVariable Long deviceId) {
        ...
        log.info(String.format("request latest telemetry-service for device id '%s'", deviceId));
        return ResponseEntity.ok(telemetryDto);
    }

    @GetMapping("/devices/{deviceId}/telemetry-service")
    public ResponseEntity<List<TelemetryDto>> getTelemetryHistory(@PathVariable Long deviceId, @RequestParam("start_time") String startTime, @RequestParam("end_time") String endTime) {
        ...
        log.info(String.format("request history telemetry-service for device id %s", deviceId));
